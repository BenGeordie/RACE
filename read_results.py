...
if __name__ == '__main__':
    ...
    if dataset=='criteo':
        ...
        file_ls_1 = glob.glob(file_path+'rslt_end2end_train_race_outputembed50_savewght_2022051*') # sampling results 
    ...
    all_cols = ['tot_itr', 'val_loss', 'val_auc', 'val_binary_crossentropy',
           ...
           'val_time', 'test_time', 'run_time','lr','repetitions','concatenations','buckets','p','batch_size','accept_first_n','score_threshold','accept_prob', 'nnd1', 'nnd2','nnd3', 'nnd4','down_sampling_rate','final_train_time','final_run_time','total_time','w_morethan_1', 'w_1', 'train_size','w_0']
    ...
    cols_2_r = ['lr','repetitions','concatenations','buckets','p','batch_size','accept_first_n','score_threshold','accept_prob', 'nnd1', 'nnd2','nnd3', 'nnd4','down_sampling_rate','final_train_time','final_run_time','total_time','w_morethan_1', 'w_1', 'train_size']

    cols_3_r = ['lr','repetitions','concatenations','buckets','p','batch_size','accept_first_n','score_threshold','accept_prob', 'nnd1', 'nnd2','nnd3', 'nnd4','down_sampling_rate']
    ...
    for fi in file_ls:
        ...
        if len(glob.glob(fi+'/*.csv'))>1:
            if 'down_sampling_rate' in max_row.columns:
                df_row = pd.concat([max_row[cols_1].reset_index(drop=True, inplace=False),val_metric.iloc[[0],:][cols_2_r].reset_index(drop=True, inplace=False)],axis=1)
            else:
            ...
            if 'down_sampling_rate' in max_row.columns:
                df_row = pd.concat([max_row[cols_1].reset_index(drop=True, inplace=False),val_metric.iloc[[0],:][cols_3_r].reset_index(drop=True, inplace=False)],axis=1)
            else:
    ...
    cols_to_show = ['tot_itr', 'val_loss', 'val_auc', 'val_binary_crossentropy',
           'val_binary_accuracy', 'test_loss', 'test_auc',
           'test_binary_crossentropy', 'test_binary_accuracy', 'train_time',
           'val_time', 'test_time', 'run_time','lr','repetitions','concatenations','buckets','p','batch_size','accept_first_n','score_threshold','accept_prob', 'nnd1', 'nnd2','nnd3', 'nnd4','down_sampling_rate','w_morethan_1', 'w_1', 'train_size','w_0']

    tr_size = df['train_size'].values # size of subsampled data
    df['w_0'] = (n_smpl - tr_size)/n_smpl
    df[cols_to_show].to_csv('aggregated_results_'+mode+'.csv',index=False)
