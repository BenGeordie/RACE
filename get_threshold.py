...
def compute_scores(dataset: tf.data.Dataset, save_path: str, embedding_model: tf.Module, total_samples, intervals=1000):
    race = make_default_race(embedding_model, total_samples)
...
# total_samples = 32343173

# print("Movielens!")
# movielens_base = "/Users/benitogeordie/Downloads/Movielenslatest_x1/data/"
# movielens_train = load_movielens_csv(movielens_base + "train_contig.csv")
# movielens_embed = make_movielens_embedding_model()
# total_samples = 1404802
# compute_scores(movielens_train, movielens_base + "scores", movielens_embed, total_samples, intervals=3000)
...
movielens_savefiles = ['/Users/benitogeordie/Downloads/Movielenslatest_x1/data/scores_last.npy']
scores = np.concatenate([np.load(f) for f in movielens_savefiles])

# avazu_savefiles = [f'/Users/benitogeordie/Downloads/Avazu_x4/score/scores_{i}.npy' for i in range(1, 64)] + ['/Users/benitogeordie/Downloads/Avazu_x4/score/scores_last.npy']
# scores = np.concatenate([np.load(f) for f in avazu_savefiles])

# print(np.min(scores[scores[:, 0] > 0][:, 0]))
# print(np.sum(scores[:0] < 0.001), flush=True)

hist = np.histogram(scores[:,0])
plt.hist(scores[:,0], bins=100, cumulative=True)
plt.show()
